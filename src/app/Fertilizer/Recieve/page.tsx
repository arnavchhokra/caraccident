  ...
  const contractAddress = "0x716493eb2356272d642e58dff2c877cd020e664a";
	  const register = async() =>{
		if (typeof window !== "undefined") {
		  ...
		  try{
      const regis = await contra.methods.receiveFertilizer(fertilizerQuantity).send({ from: accounts[0]});
      alert('Fertilizer recieved')
		  }catch(e)
		  {
			alert(e)
		  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'left', height: '80vh', width: '330px', gap: '10px' }}>
          <Card className="w-[350px] items-center">
            <CardHeader>
              <CardTitle>Recieve Fertilizer</CardTitle>
              <CardDescription>Recieve Fertilizer in one-click.</CardDescription>
            </CardHeader>
            <CardContent>
              <form>
                <div className="flex flex-col space-y-1.5 mb-2">
                  <Label htmlFor="fertilizerQuantity">ID of Fertilizer</Label>
                  <Input
